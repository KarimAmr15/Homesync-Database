...
namespace Milestone3
{
    public partial class AssignUser : System.Web.UI.Page
    {
        ...
        protected void btnAssign_Click(object sender, EventArgs e)
        {
            ...
            AssignUser.Parameters.Add(new SqlParameter("@event_id", eventid));
            AssignUser.Parameters.Add(new SqlParameter("@user_id", userid));


            try
            {
                conn.Open();
                int rowsAffected = AssignUser.ExecuteNonQuery();
                conn.Close();

                if (rowsAffected > 0)
                {
                    outputlabel.Text = "User Assigned Successfully";
                }
                else
                {
                    outputlabel.Text = "Failed To Assign User";
                }
            }
            catch (SqlException ex)
            {
